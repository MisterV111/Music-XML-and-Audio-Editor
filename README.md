...
A powerful Streamlit-based application designed for musicians and audio producers interested in efficiently editing synced music files (Score, Tempo, and Audio files). This tool enables users to edit audio files using natural language commands—leveraging OpenAI APIs—to request that specific sections of a song be removed or retained, based on the score and tempo information. Ideal for play engines that display music notation alongside audio backing tracks, users can preview and download the edited audio in both WAV and OGG formats. Future updates will introduce additional features, including tempo slowdown, pitch and key signature adjustments for both audio and score files, downloading edited score files (MusicXML) and tempo files (XML), and automatic tempo map generation to ensure seamless synchronization for interactive music learning platforms.
